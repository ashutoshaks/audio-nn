...
import os
from lru import LRU
from .library.utility.audio_utils import compute_melgram
...
class Cifar10AudioClassifier(object):
    ...
    def load_model(self, model_dir_path):
        config_file_path = Cifar10AudioClassifier.get_config_file_path(
            model_dir_path)
        params_file_path = Cifar10AudioClassifier.get_params_file_path(
            model_dir_path)
    ...
    def fit(self, audio_path_label_pairs, model_dir_path, batch_size=64, epochs=20, test_size=0.2,
        ...
        np.save(model_dir_path + '/' +
                Cifar10AudioClassifier.model_name + '-history.npy', history)
...
class Cifar10AudioSearch(Cifar10AudioClassifier):
    ...
    def query(self, audio_path, top_k=10, skip_exact_match=True):
        ...
        result = sorted(self.database, key=lambda x: self.distance(
            x[1], vec, skip_exact_match),  reverse=False)
...
class Cifar10AudioRecommender(Cifar10AudioSearch):
    ...
    def recommend(self, limits=10):
        ...
        for song in self.history:
            similar_songs = self.query(
                song, top_k=limits // 2, skip_exact_match=True)
